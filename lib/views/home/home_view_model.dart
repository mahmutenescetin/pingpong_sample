...
class HomeViewModel extends BaseViewModel {
  ...
  void _initConnectivity() {
    ...
    Connectivity().checkConnectivity().then((result) {
      final newIsOnline = result != ConnectivityResult.none;
    ...
    Connectivity().onConnectivityChanged.listen((result) {
      ...
      if (isOnline != newIsOnline) {
        ...
        notify();
        if (isOnline) {
          fetchActivity(forceOnline: true);
        }
      }
    });
  }
  ...
  Future<void> fetchActivity({bool forceOnline = false}) async {
    isLoaded = false;
    notify();
    final connectivity = await Connectivity().checkConnectivity();
    final currentOnline =
        connectivity != ConnectivityResult.none || forceOnline;
    if (currentOnline) {
